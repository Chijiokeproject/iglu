...
resource "aws_route53_record" "certificate_validation" {
  for_each = var.tools_acm_certificate_arn == null && var.create_route53_records ? toset([
    local.nexus_fqdn,
    local.sonarqube_fqdn,
    local.grafana_fqdn,
  ...
  name = one([
    for option in aws_acm_certificate.tools[0].domain_validation_options :
